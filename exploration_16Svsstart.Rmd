...
No. I should have known that wouldn't fit.

Maybe try fitting power law? It looks more like that case anyway.

```{r}
power.nls = nls(n16S_avg ~ k*start_avg^(-n),
                start = list(k = 2, n = 0.01),
                data=paprica.gr.asv)
```
...
# Fit power law
library(igraph)
pl.fit = fit_power_law(paprica.gr.asv$start_avg)
# how to visualize/evaluate?
```

Spearman rank correlation

```{r}
spear.test = cor.test(paprica.gr.asv$start_avg, paprica.gr.asv$n16S_avg, method="spearman")
spear.test
```
