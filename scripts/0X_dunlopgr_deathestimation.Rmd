...
Are linear and logarithmic death estimates comparable in some way?
...
for (l in as.character(unique(death_window$Label))) {
  ...
  this_row <- bind_cols(Label = as.character(death_label$Label),
                        ...
                        decay = "logarithmic")
  death_metrics_exp <- bind_rows(death_metrics_exp, this_row)
}

# Linear estimates
death_metrics_linear <- data.frame()
for (l in as.character(unique(death_linear_falsepos5$Label))) {
  ...
  this_row <- bind_cols(Label = as.character(death_label$Label),
                        ...
                        start_abund = start_abund, end_abund = end_abund, change_abund = change_abund,
                        decay = "linear")
  death_metrics_linear <- bind_rows(death_metrics_linear, this_row)
}

# Combine
death_metrics <- bind_rows(death_metrics_exp, death_metrics_linear)
```
