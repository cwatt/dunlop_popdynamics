...
# Fit model
# Use multiple starting points to avoid finding a local but not global maxima
S17y.mixfit1 <- normalmixEM(log(S17y$k), lambda = .5, mu = c(-2, -1), sigma = 0.3)
...
Why is C3 fast highly signifiant? The data almost entirely overlaps. In any case, it seems that the results support the hypothesis. "Slow strategists" shift growth rate more in response to C availability.


# Exploration

Are there any other variables that group with growth rates?
...
growth.paprica.asv.label %>%
  ggplot(aes(x=log(k), y=log(start_abund), color=label)) +
  geom_point(alpha=0.5) +
  facet_wrap(Soil~Amendment) +
  theme_test()

growth.paprica.asv.label %>%
  mutate(label = fct_relevel(label, "slow", "fast")) %>%
  ggplot(aes(y=log(start_abund), color=label)) +
  geom_boxplot() +
  facet_wrap(Soil~Amendment) +
  theme_test()
```
...
growth.paprica.asv.label %>%
  ...
  ggplot(aes(x=log(k), y=log(change_abund), color=label)) +
  geom_point(alpha=0.5) +
  facet_wrap(Soil~Amendment) +
  theme_test()
...
growth.paprica.asv.label %>%
  mutate(change_abund = end_abund-start_abund) %>%
  mutate(label = fct_relevel(label, "slow", "fast")) %>%
  ggplot(aes(y=log(change_abund), color=label)) +
  geom_boxplot() +
  facet_wrap(Soil~Amendment) +
  theme_test()
```

There does seem to be some clustering here... Particularly in thewater control. C-amended treatment looks more continuous.
...
```{r}
growth.paprica.asv.label %>%
  ggplot(aes(x=log(k), y=n16S, color=label)) +
  geom_point(alpha=0.5) +
  facet_wrap(Soil~Amendment) +
  theme_test()
...
growth.paprica.asv.label %>%
  mutate(label = fct_relevel(label, "slow", "fast")) %>%
  ggplot(aes(y=n16S, color=label)) +
  geom_boxplot() +
  facet_wrap(Soil~Amendment) +
  theme_test()
```

Wider variation in one group than another?
...
```{r}
growth.paprica.asv.label %>%
  ggplot(aes(x=log(k), y=genome_size, color=label)) +
  geom_point(alpha=0.5) +
  facet_wrap(Soil~Amendment) +
  theme_test()
...
growth.paprica.asv.label %>%
  mutate(label = fct_relevel(label, "slow", "fast")) %>%
  ggplot(aes(y=genome_size, color=label)) +
  geom_boxplot() +
  facet_wrap(Soil~Amendment) +
  theme_test()
```
...
I'm not seeing many clear clusters except for in change abundance in water control soils. Maybe C-amendment makes the lines between strategies "blurrier"? It's possible we haven't measured variables that may help differentiate further.
