...
plot1 <- growth.asv %>%
  ...
  labs(title = "Overall", x="Specific growth rate (units)", y="16S rRNA copy number") +
  theme_classic() +
...
plot2 <- growth.asv %>%
  ...
  labs(subtitle="Treatments", x="Specific growth rate, ln (units)", y="16S rRNA copy number") +
  theme_test() +
...
Change in abundance vs 16S copy number
...
plot3 <- growth.asv %>%
  ...
  facet_wrap(~Treatment) +
  labs(x="Specific growth rate (units), ln", y="Change in abundance, ln") +
  theme_test() +
  theme(text = element_text(size=9),
        strip.text.x = element_text(size = 9, hjust = 0),
        strip.background = element_blank())
