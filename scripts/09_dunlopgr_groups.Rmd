...
3. slow vs intermediate in cropped
4. fast vs intermediate in cropped
5. slow in water amendment, cropped vs successional
6. fast in water amendment, cropped vs successional
7. intermediate vs fast in resources, cropped vs successional
8. intermediate vs slow in resources, cropped vs successional
...
lm1 <- lm(log(k) ~ Amendment, filter(growth_clusters_stat, cluster=="slow" & Soil=="S17"))
...
lm2 <- lm(log(k) ~ Amendment, filter(growth_clusters_stat, cluster=="fast" & Soil=="S17"))
hist(resid(lm2)) # normal 
plot(predict(lm2), resid(lm2)) # variance OK w/log transform
pval2 <- anova(lm2)$`Pr(>F)`[1]

# Slow vs intermediate cropped
lm3 <- lm(log(k) ~ cluster, filter(growth_clusters, (cluster=="slow" | cluster=="intermediate") & Soil=="C3"))
hist(resid(lm3)) # normal 
plot(predict(lm3), resid(lm3)) # variance bad w/ or w/o log
# Switch to welch t-test (unequal variances)
wt3 <- t.test(log(k) ~ cluster, filter(growth_clusters, (cluster=="slow" | cluster=="intermediate") & Soil=="C3"))
pval3 <- wt3$p.value

# Fast vs intermediate cropped
lm4 <- lm(k ~ cluster, filter(growth_clusters, 
                        (cluster=="fast" | cluster=="intermediate") & Soil=="C3"))
...
kw4 <- kruskal.test(k ~ cluster, filter(growth_clusters, (cluster=="fast" | cluster=="intermediate") & Soil=="C3"))
pval4 <- kw4$p.value
...
pvals_df <- data.frame(test=c(1, 2, 3, 4, 5, 6), pval=c(pval1, pval2, pval3, pval4, pval5, pval6))
pvals_adj <- p.adjust(pvals_df$pval, method="BH", n=6)
...
# Uncertain how comparable these are

# Cropped vs successional, slow resources
lm7 <- lm(log(k) ~ Soil, filter(growth_clusters, Amendment=="Y" & (cluster=="slow" | cluster=="intermediate")))
hist(resid(lm7)) # normalish w/log 
plot(predict(lm7), resid(lm7)) # variance BAD
# Switch to Welch's t-test (unequal variances)
wt7 <- t.test(log(k) ~ Soil, filter(growth_clusters, Amendment=="Y" & (cluster=="slow" | cluster=="intermediate")))
pval7 <- wt7$p.value

# Cropped vs successional, fast resources
lm8 <- lm(k ~ Soil, filter(growth_clusters, Amendment=="Y" & (cluster=="fast" | cluster=="intermediate")))
hist(resid(lm8)) # normalish w/log 
plot(predict(lm8), resid(lm8)) # variance bad w/ or w/o log
# Switch to Welch's t-test (unequal variances)
wt8 <- t.test(log(k) ~ Soil, filter(growth_clusters, Amendment=="Y" & (cluster=="fast" | cluster=="intermediate")))
pval8 <- wt8$p.value
```
...
# Replacement vs shift

Dan: The ‘replacement’ hypothesis makes the prediction that if we observe the same slow ASV in multiple soils, its growth rate will not vary much. The ‘shift’ hypothesis predicts that if we observe the same slow ASV in multiple soils it will be slower in the low resource soils and faster in the high resource soils. If we can find a minimal set of ASVs that co-occur, and if these ASVs do or do not change their growth rate in response to resources, then we can determine whether ‘shift’ or replacement is the more likely explaination.
...
```{r, eval=FALSE}
ggsave(plot_clusterk, filename = "../figures/suppfig_clusterk.svg", units="mm", width=140, height=100, device="svg")
```
