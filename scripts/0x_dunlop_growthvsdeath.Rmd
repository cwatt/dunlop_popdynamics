...
growth_death <- inner_join(growth, death) %>% 
  select(Label, Soil, Amendment, Replicate, Domain:Genus, ASV,
         ...
         death_rate, death_start_day, death_end_day, death_start_abund, death_end_abund, death_change_abund, death_length, decay,
         edge_num:n16S)
```
...
```{r}
death_count_log <- growth_death_log %>%
  group_by(Soil, Amendment) %>% 
  summarize(num_est = n()) %>%
  ungroup()
...
```{r}
death_count_lin <- growth_death_lin %>% 
  group_by(Soil, Amendment) %>% 
  summarize(num_est = n()) %>%
  ungroup()
...
### Death start, end, length

Overall:

```{r}
death_days <- growth_death %>%
  group_by() %>% 
