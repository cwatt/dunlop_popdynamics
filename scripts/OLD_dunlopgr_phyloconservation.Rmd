...
S17y.growth.crlg <- data.frame(readRDS("../rdata.files/archive/S17Y.mantelcorrelog.rds")$mantel.res) %>%
  mutate(Soil="Successional", Amend="Y")

S17n.growth.crlg <- data.frame(readRDS("../rdata.files/archive/S17N.mantelcorrelog.rds")$mantel.res) %>% 
  mutate(Soil="Successional", Amend="N")

C3y.growth.crlg <- data.frame(readRDS("../rdata.files/archive/C3Y.mantelcorrelog.rds")$mantel.res) %>% 
  mutate(Soil="Cropped", Amend="Y")

C3n.growth.crlg <- data.frame(readRDS("../rdata.files/archive/C3N.mantelcorrelog.rds")$mantel.res) %>% 
...
```{r, results="show"}
# plot correlograms
plot1 <- ggplot(data=growth.crlg, aes(x=class.index, y=Mantel.cor)) +
...
ggsave(plot1, file="figures/suppfig_phylcons.png", width=140, height=80, units="mm")
```
...
# Overall
p1 = ucosm.growth.asv %>%
  ggplot(aes(x=log(k), y=fct_reorder(Phylum, -k), fill=Amendment)) +
  geom_boxplot() +
