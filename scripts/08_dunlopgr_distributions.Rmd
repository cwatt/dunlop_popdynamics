...
ggsave(plot, filename="../figures/fig_kdistributions.svg", units="mm", width=185, height=60, device="svg")
```
...
lag_dist <- growth.asv %>%
  mutate(Soil = fct_recode(Soil, Cropped = "C3", Successional = "S17"),
         ...
         Amendment = fct_recode(Amendment, 'Resource amendment' = "Y", 'Water amendment' = "N")) %>%
  ggplot() +
  geom_density(aes(x=start_day, fill=Soil), alpha=0.5) +
  scale_fill_manual(values = c("#676366", "#FFFFFF")) +
  facet_wrap(~Amendment, scales="fixed") +
...
#ggsave(plot, filename="../figures/fig_distributions.svg", units="mm", width=190, height=60, device="svg")
```
...
lag_dist <- growth.asv %>%
  ...
  ggplot() +
  geom_density(aes(x=change_abund, fill=Soil), alpha=0.5) +
  scale_fill_manual(values = c("#676366", "#FFFFFF")) +
  facet_wrap(~Amendment, scales="fixed") +
...
lag_dist <- growth.asv %>%
  mutate(Soil = fct_recode(Soil, Cropped = "C3", Successional = "S17"),
         Amendment = fct_relevel(Amendment, c("N", "Y")),
         Amendment = fct_recode(Amendment, 'Resource amendment' = "Y", 'Water amendment' = "N"),
         length_growth = end_day - start_day) %>%
  ggplot() +
  geom_density(aes(x=length_growth, fill=Soil), alpha=0.5) +
  scale_fill_manual(values = c("#676366", "#FFFFFF")) +
  facet_wrap(~Amendment, scales="fixed") +
  labs(y="Kernel density", x="Length of growth (days)") +
  theme_classic() +
  theme(text = element_text(size=9),
        strip.text.x = element_text(size = 10, hjust = 0),
        legend.position = "right",
