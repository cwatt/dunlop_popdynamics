...
plot1 <- growth.asv %>%
  filter(Soil=="C3", Amendment=="N") %>%
  mutate(Treatment = as_factor(paste0(Soil, Amendment)),
         Treatment = fct_relevel(Treatment, c("C3N", "S17N", "C3Y", "S17N")), # BROKEN
         Treatment = fct_recode(Treatment, 'Cropped, water'="C3N", 'Cropped, resources'="C3Y", 'Successional, water'="S17N", 'Successional, resources'="S17Y")) %>%
  ggplot() +
  geom_density(aes(x=log(k))) +
  labs(y="Kernel density", x="Specific growth rate (units), ln") +
  stat_function(fun = dnorm, n = 100, args = list(mean = -3.165505, sd = 0.1338452)) +
  stat_function(fun = dnorm, n = 100, args = list(mean = -1.402565, sd = 0.1338452)) +
  theme_test() +
