...
# Clean up data frame
ucosm.clean = ucosm.log %>%
  select(Soil, Amendment, Replicate, Day, ASV, Domain, Phylum, Class, Order, Class, Family, Genus, norm.abund = abund, log.abund = abund.log)
```
...
p4 = ggplot(p4.data, aes(x=Day, y=log.abund)) +
  ...
  labs(title=paste(unique(p4.data$Phylum), ", ", unique(p4.data$Family), sep=""), y="ln normalized abundance") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
        title = element_text(size=9),
