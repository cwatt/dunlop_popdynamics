...
Unfortunately, I didn't randomize my DNA extractions, so this is confounded by samples processed in one kit being more similar to one another in other ways ie plot location, point in time series, treatment.
...
Summary: Min, max, average, standard deviation of percent internal standard:
...
Nope, use Spearman correlation instead (non-parametric).
...
These will be edited in adobe illustrator for minor adjustments to aesthetics.
...
p1 = intstd %>%
  ...
  labs(x="DNA concentration (ng/\u03BCL)", y="Internal standard (log, % of reads)") +
...
```{r, eval = FALSE}
library(Cairo)
ggsave(plot = p1, file = "figures/fig_dnavsintstd.pdf", device=cairo_pdf, width = 90, height = 90, unit="mm")
```
