...
dna = read_csv("growthrate_dnaext.csv") %>% 
  select("Sample" = SampleID, "dna_conc" = Conc.) %>%
...
for (s in c("C3","S17")) {
  for (a in c("N","Y")) {
    ...
    this.lm  <- lm(dna_conc_avg ~ Day, data=this.data)
    hist(residuals(this.lm), main = paste(s, a))
    plot(residuals(this.lm), predict(this.lm), main = paste(s, a))
    this.lm2 <- lm(log(dna_conc_avg) ~ Day, data=this.data) # try log transform too
    ...
    this.row <- data.frame("Soil"=s, "Amendment"=a, "slope_est"=summary(this.lm)$coefficients[2,1], "slope_pvalue"=summary(this.lm)$coefficients[2,4])
...
plot_ordination(physeq.day0, day0.ord, color="Type") +
  facet_wrap(~Soil) +
  theme_test()
```
...
# Weighted unifrac
physeq.ord.wunifrac = ordinate(physeq, method="NMDS", distance="wunifrac")

# Compare soils
plot_ordination(physeq, physeq.ord.wunifrac, color="Soil") +
  facet_grid(~Amendment) +
...
plot_ordination(physeq, physeq.ord.wunifrac, color="Amendment") +
  facet_grid(~Soil) +
  labs(title="Weighted Unifrac") +
  theme_test() +
  theme(strip.background = element_blank(),
...
# Unifrac
physeq.ord.unifrac = ordinate(physeq, method="NMDS", distance="unifrac")

# Compare soils
plot_ordination(physeq, physeq.ord.unifrac, color="Soil") +
  facet_grid(~Amendment) +
  labs(title="Unifrac") +
  theme_test() +
  theme(strip.background = element_blank(),
...
plot_ordination(physeq, physeq.ord.unifrac, color="Amendment") +
  facet_grid(~Soil) +
  labs(title="Unifrac") +
  theme_test() +
  theme(strip.background = element_blank(),
