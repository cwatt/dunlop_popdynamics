...
It looks like 5 bins is definitely too few. I'll choose 8 bins because they best capture the distribution shape/bimodal nature of water amended data.
...
```{r, results="show"}
width = (max(log(gr.estavg$avgk))-min(log(gr.estavg$avgk)))/8
...
print(paste("Bin 7 boundaries:", kmin+width*6, kmin+width*7))
print(paste("Bin 8 boundaries:", kmin+width*7, "and greater"))
```

```{r}
# Bin data based on boundaries chosen and log k
gr.est.bins = gr.est %>% mutate(bin = ifelse(log(k) < kmin+width, 1, ifelse(log(k) < kmin+width*2, 2, ifelse(log(k) < kmin+width*3, 3, ifelse(log(k) < kmin+width*4, 4, ifelse(log(k) < kmin+width*5, 5, ifelse(log(k) < kmin+width*6, 6, ifelse(log(k) < kmin+width*7, 7, 8))))))))
...
The growth rate distribution graphs show distinct bimodal distributions in the water control treatment. This seems to indicate differing populations, which I want to explore further.
...
Following this advice: https://stackoverflow.com/questions/25264461/find-local-minimum-in-bimodal-distribution-with-r

Find local minima by finding where dx/dy = 0 for each treatment distribution.

```{r}
# Find local minima
thresholds <- data.frame()
for (s in c("C3", "S17")) {
  df <- filter(gr.estavg, Soil==s, Amendment=="N")
  d <- density(log(df$avgk))
  o <- optimize(approxfun(d$x,d$y),interval=c(-3,-1))$minimum # find where dx/dy = 0 within given interval
  print(ggplot(df, aes(x=log(avgk))) +
    geom_density() +
    geom_vline(xintercept=o) +
    labs(title=paste(s, "N,", "Threshold:", o)) +
    theme_test())
  this.row <- data.frame(Soil=s, Amendment="N", threshold=o)
  thresholds <- bind_rows(thresholds, this.row)
}
```

Label ASVs based on threshold:

```{r}
# Labeling based on thresholds
gr.estavg.group <- gr.estavg %>% 
  filter(Amendment=="N") %>% # water control data only
  mutate(group = ifelse(Soil=="C3" & log(avgk) < thresholds[thresholds$Soil=="C3",]$threshold, "slow", 
                        ifelse(Soil=="S17" & log(avgk) < thresholds[thresholds$Soil=="S17",]$threshold, "slow", "fast"))) # use thresholds defined above to categorize data based on growth rates
...
### "Fast" vs "slow in unamended soils

Is there  a difference in the number of taxa in these groups in the cropped and successional water-amended soil?
...
gr.est.group = gr.est %>% 
  ...
  mutate(group = ifelse(Soil=="C3" & log(k) < thresholds[thresholds$Soil=="C3",]$threshold, "slow", 
                        ifelse(Soil=="S17" & log(k) < thresholds[thresholds$Soil=="S17",]$threshold, "slow", "fast")))
...
gr.est.group.prop = gr.est.group.count %>%
  ...
  mutate(prop_fastslow = fast/(fast+slow))
```
...
```{r, eval=FALSE}
# Make a nice graph for poster
poster = gr.est.group.prop %>%
...
ggsave(poster, file="figures/fig_propfastslow_poster.pdf", device="pdf", width=2.2, height=2.7, units="in")
```


**Statistics**

Compare **frequency** of taxa in "fast" vs "slow" group in each soil.
...
```{r}
group.count.lmer = lmer(log(n) ~ group*Soil + (1|Replicate), data=gr.est.group.count) # this should be 1|sample not replicate
hist(residuals(group.count.lmer))
plot(residuals(group.count.lmer), predict(group.count.lmer))
```

The variance looks bad. Try non-parametric test.

Try paired Welch's t tests instead.

```{r}
group.count.wtest = t.test(gr.est.group.count[gr.est.group.count$Soil=="C3" & gr.est.group.count$group=="fast",]$n,
                           gr.est.group.count[gr.est.group.count$Soil=="C3" & gr.est.group.count$group=="slow",]$n,
                           paired=TRUE, var.equal=FALSE)
group.count.wtest

group.count.wtest = t.test(gr.est.group.count[gr.est.group.count$Soil=="S17" & gr.est.group.count$group=="fast",]$n,
                           gr.est.group.count[gr.est.group.count$Soil=="S17" & gr.est.group.count$group=="slow",]$n,
                           paired=TRUE, var.equal=FALSE)
group.count.wtest
```

Not significant. Likely low power and high variability.
...
# t-test (because people complain when I use a linear regression instead of a t-test...)
group.prop.ttest = t.test(gr.est.group.prop[gr.est.group.prop$Soil=="C3",]$prop_fastslow,
                          gr.est.group.prop[gr.est.group.prop$Soil=="S17",]$prop_fastslow,
                          data = gr.est.group.prop)
group.prop.ttest
```
...
Will evaluate this further with 16S copy number corrected data, since you'd imagine fast taxa might be more likely to ahve more copies and bias the results.
...
p1 = gr.estavg %>%
  mutate(Soil = recode_factor(Soil, S17="Successional", C3="Cropped"),
         Amendment = recode_factor(Amendment, N="Water control", Y="C-amended")) %>%
  ggplot(aes(x=log(avgk), fill=Soil)) +
  geom_density(alpha=0.5) +
  facet_wrap(~Amendment, ncol=1) +
  scale_fill_manual(values=c(NA, "gray")) +
  labs(y="Kernel density", x=expression("Specific growth rate (ln, day"^{-1}*")")) +
  theme_test() +
...
p.poster = gr.estavg %>%
  mutate(Soil = recode_factor(Soil, S17="Successional", C3="Cropped"),
         Amendment = recode_factor(Amendment, N="Water-amended", Y="C-amended")) %>%
  ggplot(aes(x=log(avgk), fill=Soil)) +
  geom_density(alpha=0.5) +
  facet_wrap(~Amendment, ncol=1) +
  scale_fill_manual(values=c(NA, "gray")) +
...
ggsave(plot=p.poster, file="figures/fig_habitatdist_poster.pdf", device=pdf, width=4.4, height=4.4, units="in")
```
