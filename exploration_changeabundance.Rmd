...
Strong skew in variances. This can be OK as long as we don't build interaction terms into the model.

```{r}
# Fit model without interactions
abundchange1.asv.lmer2 = lmer(log(change_abund) ~ Soil+Amendment + (1|ASV), data=abundchange1.asv)

# Check model assumptions
hist(residuals(abundchange1.asv.lmer2)) # normality
plot(residuals(abundchange1.asv.lmer2), predict(abundchange1.asv.lmer)) # variances
```

Want to consult statistician.
