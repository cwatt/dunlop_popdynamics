...
```{r}
# Cropped
field.group.ind.C3.lmer = lmer(log(corr_abund) ~ group + Block + (1|Replicate), data=field.group.n16S[field.group.n16S$Soil=="C3",]) # fit model
...
# Successional
field.group.ind.S17.lmer = lmer(log(corr_abund) ~ group + Block + (1|Replicate), data=field.group.n16S[field.group.n16S$Soil=="S17",]) # fit model
...
p1 = paprica.gr.avgtrt %>%
  ...
  geom_point(shape=1) +
  labs(title="Overall", x=expression("Specific growth rate (ln normalized reads day"^{-1}*")"), y="Estimated 16S copy number") +
  theme_test() +
...
p2 = paprica.gr.avgtrt %>%
  ...
  labs(x=expression("Specific growth rate (ln normalized reads day"^{-1}*")")) +
  theme_test() +
  theme(axis.line = element_line(colour = "black"),
        strip.background = element_blank(),
        strip.text = element_text(size=10),
...
copynum.plot = plot_grid(p1, p2, axis="t", align="h", labels=c("a","b"), label_size=10)
copynum.plot
```
...
```{r, eval=FALSE}
ggsave(plot=copynum.plot, fil="figures/fig_copynum.pdf", device=pdf, width=190, height=100, unit="mm")
```
