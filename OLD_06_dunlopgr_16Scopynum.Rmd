...
```{r, eval=FALSE}
# Save merged paprica data
saveRDS(paprica, file="rdata.files/paprica.est.rds")
```
...
```{r}
# Visualize
paprica.gr.avgall %>% ggplot(aes(x=log(avgk), y=n16S)) +
  geom_point(aes(color=Phylum)) +
  #geom_smooth(method="lm") +
  labs(x="avg specific growth rate", y="estimated 16S copies") +
  theme_test()

paprica.gr.avgsoil %>% ggplot(aes(x=log(avgk), y=n16S)) +
  geom_point(aes(color=Phylum)) +
  #geom_smooth(method="lm") +
  facet_wrap(~Soil) +
  labs(x="avg specific growth rate", y="estimated 16S copies") +
...
paprica.gr.avgtrt %>% ggplot(aes(x=log(avgk), y=n16S)) +
  geom_point(aes(color=Phylum)) +
  #geom_smooth(method="lm") +
...
# Tests
n16S.all.spearperm = spearman_permute(paprica.gr.avgtrt$n16S, paprica.gr.avgtrt$avgk)

n16S.C3y.spearperm = spearman_permute(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="C3" & paprica.gr.avgtrt$Amendment=="Y",]$n16S, 
                                      log(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="C3" & paprica.gr.avgtrt$Amendment=="Y",]$avgk))

n16S.C3n.spearperm = spearman_permute(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="C3" & paprica.gr.avgtrt$Amendment=="N",]$n16S, 
                                      log(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="C3" & paprica.gr.avgtrt$Amendment=="N",]$avgk))

n16S.S17y.spearperm = spearman_permute(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="S17" & paprica.gr.avgtrt$Amendment=="Y",]$n16S, 
                                       log(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="S17" & paprica.gr.avgtrt$Amendment=="Y",]$avgk))

n16S.S17n.spearperm = spearman_permute(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="S17" & paprica.gr.avgtrt$Amendment=="N",]$n16S, 
                                       log(paprica.gr.avgtrt[paprica.gr.avgtrt$Soil=="S17" & paprica.gr.avgtrt$Amendment=="N",]$avgk))

# Multiple test correction
n16S.spearperm.pvals = c(n16S.all.spearperm[1], n16S.C3y.spearperm[1], n16S.C3n.spearperm[1], n16S.S17y.spearperm[1], n16S.S17n.spearperm[1])
n16S.spearperm.padj = p.adjust(n16S.spearperm.pvals, method="hochberg", n=length(n16S.spearperm.pvals))

# Estimates
n16S.spearperm.rhos = c(n16S.all.spearperm[2], n16S.C3y.spearperm[2], n16S.C3n.spearperm[2], n16S.S17y.spearperm[2], n16S.S17n.spearperm[2])

# Results table
n16S.spearperm.results = data.frame(Soil = c("overall", rep("C3", 2), rep("S17", 2)), Amendment=c("overall","Y","N","Y","N"), p.adj=n16S.spearperm.padj, rho = n16S.spearperm.rhos)
...
ggsave(plot=p1.poster, file="figures/fig_copynum_poster.pdf", device=pdf, width=4.4, height=2.2, unit="in")
```
...
### ASV abundances, corrected for copy number

```{r}
p4 = field.group.n16S %>% 
  mutate(Soil = fct_relevel(Soil, "S17"), 
         Soil = recode_factor(Soil, S17="Successional", C3="Cropped")) %>%
  ggplot(aes(x=Soil, y=log(corr_abund), fill=group)) +
  geom_boxplot() +
  #geom_jitter(alpha=0.2) +
  scale_fill_manual(values=c(NA, "gray")) +
  labs(title="Individual ASV abundances", x="", y="natural log internal-standard normalized abundance", color="Block") +
...
```{r, eval=FALSE}
ggsave(plot=p4, file="figures/fig_indabund.pdf", device=pdf, width=90, height=90, units="mm")
```